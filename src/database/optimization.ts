...
export async function createPerformanceIndexes(): Promise<void> {
  try {
    ...
    await db.collection('payment_transactions').createIndex({ externalId: 1 }, { unique: true, sparse: true });
...
async function logIndexStatus(): Promise<void> {
  try {
    const db = getDb();
    const collections = ['users', 'orders', 'products', 'payment_transactions', 'categories', 'notifications'];
