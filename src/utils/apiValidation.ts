 ...
 */
export function successResponse<T>(data: T, meta?: { page?: number; limit?: number; total?: number }): ApiResponse<T> {
...
export function handleValidationErrors(req: Request, res: Response, next: NextFunction) {
  ...
  if (!errors.isEmpty()) {
    ...
    return res.status(400).json(
      errorResponse('Validation failed', 'VALIDATION_ERROR', errorDetails)
    );
  }
...
export const validateCreateUser = [
  body('telegramId')
    .isInt({ min: 1 })
    .withMessage('telegramId must be a positive integer'),
  body('username')
    .optional()
...
export const validateCreateProduct = [
  ...
  body('allowPreorder')
    .isBoolean()
    .withMessage('allowPreorder must be a boolean'),
  body('preorderNote')
    .optional()
    .isLength({ max: 500 })
...
export function sanitizeInput(req: Request, _res: Response, next: NextFunction) {
  ...
  const cleanString = (val: string) => val
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (current.count >= max) {
      return res.status(429).json(
        errorResponse(message || 'Too many requests', 'RATE_LIMIT_EXCEEDED')
      );
    }
...
export function globalErrorHandler(error: any, req: Request, res: Response, next: NextFunction) {
  ...
  if (error.code === 11000) {
    ...
    return res.status(409).json(
      errorResponse(`Duplicate value for ${field}`, 'DUPLICATE_ERROR', { field })
    );
  }
  ...
  if (error.name === 'ValidationError') {
    ...
    return res.status(400).json(
      errorResponse('Database validation failed', 'DB_VALIDATION_ERROR', details)
    );
  }
  ...
  if (error.name === 'CastError') {
    return res.status(400).json(
      errorResponse('Invalid ID format', 'INVALID_ID', { field: error.path })
    );
  }
