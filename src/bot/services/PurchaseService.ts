...
Are you sure you want to proceed with this purchase?`;
  ...
  static async completeOrder(orderId: string): Promise<boolean> {
    try {
      const updatedOrder = await OrderRepository.updateOrderStatus(orderId, 'delivered');
  ...
  static async cancelOrder(orderId: string): Promise<boolean> {
    try {
      const updatedOrder = await OrderRepository.updateOrderStatus(orderId, 'cancelled');
