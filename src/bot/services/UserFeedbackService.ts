...
export class UserFeedbackService {
  ...
  static async handleError(ctx: MyContext, error: Error, context: string): Promise<void> {
    ...
    switch (context) {
      case 'product_loading':
        errorMessage +=
          '🛍️ We had trouble loading the products.\n\n' +
          '📋 *What you can do:*\n' +
          '• Try refreshing the product list\n' +
          '• Check your internet connection\n' +
          '• Return to main menu and try again\n\n';

        recoveryOptions = KeyboardFactory.confirmationDialog('view_categories', 'main_menu');
        break;
  ...
  static async provideContextualHelp(ctx: MyContext, currentAction: string): Promise<void> {
    ...
    const helpKeyboard = KeyboardFactory.helpMenu();

    switch (currentAction) {
      case 'browsing_products':
        helpMessage +=
          '🛍️ *Browsing Products Help*\n\n' +
  ...
  static async collectFeedback(ctx: MyContext, experienceType: string): Promise<void> {
    const feedbackMessage =
      messages.createHeader('We Value Your Feedback', '⭐') +
      '\n' +
    ...
    const feedbackKeyboard = new InlineKeyboard()
      ...
      .text('⭐⭐⭐⭐ Good', `feedback_4_${experienceType}`)
      .row()
      .text('⭐⭐⭐ Average', `feedback_3_${experienceType}`)
      .text('⭐⭐ Poor', `feedback_2_${experienceType}`)
      .row()
      .text('⭐ Very Poor', `feedback_1_${experienceType}`)
      .text('💭 Add Comment', `feedback_comment_${experienceType}`)
      .row()
  ...
  static async confirmAction(
    ...
    const confirmMessage =
      messages.createHeader('Please Confirm', '❓') +
      '\n' +
      `🎯 *Action:* ${action}\n\n` +
      `📋 *Description:*\n${description}\n\n` +
    ...
    const confirmKeyboard = new InlineKeyboard()
      .text('✅ Yes, Continue', confirmCallback)
      .row()
