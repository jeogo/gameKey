...
class KeyboardFactory {
  ...
  static mainMenu(): InlineKeyboard {
    return new InlineKeyboard()
      .text("🛍️ Products", "view_categories").text("📋 Orders", "view_orders").row()
      .text("💬 Help", "help_menu");
  }
  ...
  static products(products: IProduct[], categoryId: string): InlineKeyboard {
    ...
    products.forEach(product => {
      ...
      keyboard.text(`${statusIcon} ${product.name} - $${product.price}`, `product_${product._id}`).row();
    });
  ...
  static confirmation(confirmCallback: string, cancelCallback: string): InlineKeyboard {
    return new InlineKeyboard()
      .text("✅ Confirm", confirmCallback).text("❌ Cancel", cancelCallback);
  }
  ...
  static terms(): InlineKeyboard {
    return new InlineKeyboard()
      .text("✅ I Agree", "accept_terms").text("❌ Decline", "decline_terms");
  }

  static profileMenu(): InlineKeyboard {
    return new InlineKeyboard()
      .text("📋 My Order History", "view_orders").row()
      .text("⬅️ Back", "main_menu");
  }

  static ordersWithPagination(orders: any[], page: number, totalPages: number): InlineKeyboard {
    const keyboard = new InlineKeyboard();
    orders.forEach(order => {
      keyboard.text(`Order #${order._id}`, `order_${order._id}`).row();
    });
    return keyboard.text("⬅️ Back", "main_menu");
  }

  static orderDetails(order: any): InlineKeyboard {
    return new InlineKeyboard()
      .text("⬅️ Back to Orders", "view_orders");
  }

  static paymentLink(url: string, transactionId: string): InlineKeyboard {
    return new InlineKeyboard()
      .url("💳 Pay Now", url).row()
      .text("⬅️ Back", "main_menu");
  }
