...
async function processNowPaymentsWebhook(webhookData: any): Promise<boolean> {
  try {
    const { payment_id, payment_status, order_id, price_amount, price_currency, actually_paid } = webhookData;
    ...
    if (!transaction) {
      console.error(`❌ Transaction not found for payment_id: ${payment_id}, order_id: ${order_id}`);
      return false;
    }
 ...
 */
async function handleFailedPayment(transaction: IPaymentTransaction, reason: string): Promise<void> {
  try {
 ...
 */
async function notifyPaymentIssue(
  userId: string,
  order: any,
  issue: string
): Promise<void> {
  try {
    ...
    const message = `⚠️ **ORDER STATUS UPDATE**\n\n` +
 ...
 */
async function notifyInventoryIssue(
  telegramId: number,
  order: any,
  product: any
): Promise<void> {
...
export async function checkPaymentStatusWithProvider(transactionId: string): Promise<boolean> {
  try {
    ...
    switch (transaction.paymentProvider.toLowerCase()) {
      ...
      default:
        console.warn(`⚠️ Status check not implemented for provider: ${transaction.paymentProvider}`);
        return false;
    }
 ...
 */
async function processPayPalWebhook(webhookData: any): Promise<boolean> {
 ...
 */
async function processStripeWebhook(webhookData: any): Promise<boolean> {
